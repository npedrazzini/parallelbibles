#Add the full path to "when-while-data.txt" and "when-while-matrix.txt" or start an R project in the same directory as those files
install.packages('Cairo') #if you haven't already
library(Cairo)
data<-read.table("when-while-data.txt")
matr<-read.table("when-while-matrix.txt")
...
CairoPDF(file='when-while-plots.pdf',16, 12)
